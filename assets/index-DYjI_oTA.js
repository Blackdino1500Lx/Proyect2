import"https://esm.sh/@supabase/supabase-js@2";(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))e(r);new MutationObserver(r=>{for(const i of r)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&e(o)}).observe(document,{childList:!0,subtree:!0});function n(r){const i={};return r.integrity&&(i.integrity=r.integrity),r.referrerPolicy&&(i.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?i.credentials="include":r.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function e(r){if(r.ep)return;r.ep=!0;const i=n(r);fetch(r.href,i)}})();const te="modulepreload",ae=function(t){return"/Proyect2/"+t},R={},O=function(a,n,e){let r=Promise.resolve();if(n&&n.length>0){document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),s=o?.nonce||o?.getAttribute("nonce");r=Promise.allSettled(n.map(d=>{if(d=ae(d),d in R)return;R[d]=!0;const l=d.endsWith(".css"),f=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${f}`))return;const m=document.createElement("link");if(m.rel=l?"stylesheet":te,l||(m.as="script"),m.crossOrigin="",m.href=d,s&&m.setAttribute("nonce",s),document.head.appendChild(m),l)return new Promise((w,ee)=>{m.addEventListener("load",w),m.addEventListener("error",()=>ee(new Error(`Unable to preload CSS for ${d}`)))})}))}function i(o){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=o,window.dispatchEvent(s),!s.defaultPrevented)throw o}return r.then(o=>{for(const s of o||[])s.status==="rejected"&&i(s.reason);return a().catch(i)})};function re(){const t=document.createElement("style");t.textContent=`
:root{
...
@media(max-width:640px){main{padding:1rem}header{padding:.65rem 1rem}}
  `,document.head.appendChild(t)}const D=["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"],u=new Date;function _(t){const a=new Date(u);return a.setDate(u.getDate()+(t-u.getDay())),a.toISOString().split("T")[0]}function T(t){const a=new Date(u);return a.setDate(u.getDate()+(t-u.getDay())+7),a.toISOString().split("T")[0]}function B(){const t=new Date(u.getFullYear(),0,1),a=Math.ceil(((u-t)/864e5+t.getDay()+1)/7);return`${u.getFullYear()}-W${String(a).padStart(2,"0")}`}const y=u,C={"admin@cong.com":{email:"admin@cong.com",name:"Hermano Administrador",role:"admin",group_id:"g1",password:"admin123"},"usuario@cong.com":{email:"usuario@cong.com",name:"Pedro González",role:"user",group_id:"g2",password:"user123"}};function ne(){return{groups:[{id:"g1",name:"Grupo Norte",captain:"José Ramírez"},{id:"g2",name:"Grupo Sur",captain:"María López"},{id:"g3",name:"Grupo Centro",captain:"Ana Vargas"}],users:Object.values(C).map(t=>({email:t.email,name:t.name,role:t.role,group_id:t.group_id})),meetings:[{id:"m1",title:"Reunión Entre Semana",date:_(2),time:"19:00",type:"midweek",description:"Tesoros de la Biblia · Seamos mejores maestros · Nuestra vida cristiana",created_at:Date.now()},{id:"m2",title:"Reunión Fin de Semana",date:_(6),time:"10:00",type:"weekend",description:"Discurso público · Estudio de La Atalaya",created_at:Date.now()},{id:"m3",title:"Asamblea de Circuito",date:T(6),time:"09:00",type:"special",description:"Programa completo de dos días. Traiga almuerzo.",created_at:Date.now()}],announcements:[{id:"a1",title:"Asamblea de Circuito próxima",body:"Los días 22 y 23 se celebrará la asamblea. Regístrese puntualmente.",priority:"urgent",created_at:Date.now()-864e5},{id:"a2",title:"Limpieza del Salón",body:"Este sábado a las 8:00 AM se realizará limpieza general.",priority:"normal",created_at:Date.now()-1728e5},{id:"a3",title:"Nuevo horario de verano",body:"Las reuniones entre semana iniciarán a las 7:30 PM.",priority:"info",created_at:Date.now()-2592e5}],assignments:[{id:"as1",name:"Pedro González",email:"usuario@cong.com",role:"Discurso de 5 minutos",week:B(),created_at:Date.now()},{id:"as2",name:"Ana Vargas",email:"ana@cong.com",role:"Oración inicial",week:B(),created_at:Date.now()},{id:"as3",name:"Carlos Méndez",email:"carlos@cong.com",role:"Lector de La Atalaya",week:B(),created_at:Date.now()}],reports:[{id:"r1",email:"usuario@cong.com",year:u.getFullYear(),month:u.getMonth(),hours:12,revisits:8,studies:2,videos:5,notes:"Buen mes",created_at:Date.now()},{id:"r2",email:"usuario@cong.com",year:u.getFullYear(),month:Math.max(0,u.getMonth()-1),hours:10,revisits:6,studies:1,videos:3,notes:"",created_at:Date.now()-2592e6}],cleaning:[{id:"cl1",who:"Familia González",date:_(6),notes:"Salón principal y baños",created_at:Date.now()},{id:"cl2",who:"Hermano Vega y familia",date:T(6),notes:"Cocina, jardín y estacionamiento",created_at:Date.now()}],workprogram:[{id:"wp1",title:"Pintura de la fachada",date:T(6),who:"Grupo Norte",notes:"Se necesitan voluntarios. Llevar ropa de trabajo.",created_at:Date.now()}]}}let g=null;function $(){return g}function F(t){g=t}function k(){localStorage.setItem("kharis_v3",JSON.stringify(g))}async function h(t){return[...g[t]||[]]}async function E(t,a){{const n={id:t[0]+Date.now(),...a,created_at:Date.now()};return g[t].unshift(n),k(),n}}async function z(t,a){{g[t]=g[t].filter(n=>n.id!==a),k();return}}async function H(t,a){{const n=g.reports.findIndex(e=>e.email===a.email&&e.year===a.year&&e.month===a.month);n>=0?g.reports[n]={...g.reports[n],...t}:g.reports.unshift({id:"r"+Date.now(),...t,created_at:Date.now()}),k();return}}async function M(){return g.users}async function j(){return g.groups}async function q(t,a){{const n=g.users.find(e=>e.email===t);n&&(n.group_id=a,k());return}}async function U(t,a){{const n=g.users.find(e=>e.email===t);n&&(n.role=a,k());return}}const ie=Object.freeze(Object.defineProperty({__proto__:null,del:z,get:h,getDS:$,getGroups:j,getUsers:M,ins:E,saveDS:k,setDS:F,setUserGroup:q,setUserRole:U,upsertReport:H},Symbol.toStringTag,{value:"Module"}));function oe(){const t=localStorage.getItem("kharis_v3");F(t?JSON.parse(t):ne())}async function se(t,a){{const n=C[t.toLowerCase()];if(!n)throw new Error("Usuario no encontrado");if(n.password!==a)throw new Error("Contraseña incorrecta");const r=$().groups.find(i=>i.id===n.group_id);return{...n,group:r}}}async function de(t,a,n){{if(C[a])throw new Error("El correo ya está registrado");C[a]={email:a,name:t,role:"user",group_id:null,password:n},$().users.push({email:a,name:t,role:"user",group_id:null}),k();return}}async function le(){}function ce(t){return{midweek:"Entre semana",weekend:"Fin de semana",special:"Especial / Asamblea"}[t]||t}function me(t){return{midweek:"b-sky",weekend:"b-green",special:"b-amber"}[t]||"b-gray"}function c(t,a,n=!1){const e=document.getElementById("toast");document.getElementById("t-title").textContent=t,document.getElementById("t-msg").textContent=a,e.style.borderColor=n?"var(--rose)":"var(--sky)",e.classList.add("show"),setTimeout(()=>e.classList.remove("show"),3500)}function N(t){return new Date(t+"T00:00:00").toLocaleDateString("es",{weekday:"long",year:"numeric",month:"long",day:"numeric"})}function pe(t){return new Date(t).toLocaleDateString("es",{day:"2-digit",month:"long",year:"numeric"})}function ge(){document.getElementById("auth-screen").innerHTML=`
    <div class="auth-wrap">
      <div class="auth-hero">
    ...
    </div>`,document.getElementById("tab-login").addEventListener("click",()=>P("login")),document.getElementById("tab-reg").addEventListener("click",()=>P("register")),document.getElementById("btn-login").addEventListener("click",async()=>{const t=document.getElementById("l-email").value.trim().toLowerCase(),a=document.getElementById("l-pass").value,n=document.getElementById("btn-login"),e=document.getElementById("login-err");e.style.display="none",n.innerHTML='<span class="spin"></span>',n.disabled=!0;try{const r=await se(t,a);window.__onLogin(r)}catch(r){e.textContent=r.message,e.style.display="block"}n.textContent="Iniciar sesión",n.disabled=!1}),document.getElementById("btn-reg").addEventListener("click",async()=>{const t=document.getElementById("r-name").value.trim(),a=document.getElementById("r-email").value.trim().toLowerCase(),n=document.getElementById("r-pass").value,e=document.getElementById("btn-reg"),r=document.getElementById("reg-err");if(r.style.display="none",!t||!a||!n){r.textContent="Completa todos los campos",r.style.display="block";return}e.innerHTML='<span class="spin"></span>',e.disabled=!0;try{await de(t,a,n),c("¡Cuenta creada!","Ahora puedes iniciar sesión"),P("login")}catch(i){r.textContent=i.message,r.style.display="block"}e.textContent="Crear cuenta",e.disabled=!1})}function P(t){document.getElementById("form-login").style.display=t==="login"?"block":"none",document.getElementById("form-reg").style.display=t==="register"?"block":"none",document.getElementById("tab-login").classList.toggle("active",t==="login"),document.getElementById("tab-reg").classList.toggle("active",t==="register")}function A(t){document.querySelectorAll(".page").forEach(a=>a.classList.remove("active")),document.querySelectorAll(".nt").forEach(a=>a.classList.remove("active")),document.getElementById("page-"+t)?.classList.add("active"),document.querySelector(`[data-page="${t}"]`)?.classList.add("active")}function ue(t){const a=t.role==="admin";document.getElementById("app").innerHTML=`
    <div id="install-bar">
      <span>📲</span>
    ...
    </main>`,document.querySelectorAll(".nt").forEach(n=>{n.addEventListener("click",()=>{const e=n.dataset.page;A(e),window.__loadPage(e)})}),document.getElementById("btn-logout").addEventListener("click",async()=>{await le(),window.__showAuth()}),document.getElementById("btn-close-install").addEventListener("click",()=>{document.getElementById("install-bar").classList.remove("show")})}re();oe();let p=null,b=null,G=[],Y=[],x="all";const ve=[{id:1,name:"Sector A – Residencial",lat:9.938,lng:-84.089,status:"available",group:"Grupo Norte",notes:"Casas unifamiliares"},{id:2,name:"Sector B – Comercial",lat:9.93,lng:-84.079,status:"in-progress",group:"Grupo Sur",notes:"En trabajo actualmente"},{id:3,name:"Sector C – Norte",lat:9.945,lng:-84.084,status:"completed",group:"Grupo Norte",notes:"Completado el mes pasado"},{id:4,name:"Sector D – Este",lat:9.925,lng:-84.073,status:"available",group:"Grupo Centro",notes:"Urbanización reciente"},{id:5,name:"Sector E – Sur",lat:9.918,lng:-84.09,status:"in-progress",group:"Grupo Sur",notes:"En progreso"},{id:6,name:"Sector F – Centro",lat:9.933,lng:-84.084,status:"available",group:"Grupo Centro",notes:"Área densa"}];window.__showAuth=function(){p=null,document.getElementById("app").style.display="none",document.getElementById("auth-screen").style.display="flex",ge()};window.__onLogin=async function(t){p=t,document.getElementById("auth-screen").style.display="none",document.getElementById("app").style.display="flex",ue(t),await I("dash")};window.__loadPage=I;async function I(t){const a=document.getElementById("main-content");switch(t){case"dash":return be(a);case"meetings":return V(a);case"announcements":return J(a);case"assignments":return K(a);case"programs":return fe(a);case"cleaning":return Z(a);case"workprogram":return Q(a);case"map":return ye(a);case"reports":return X(a);case"admin":return v(a)}}async function be(t){const{renderDashboard:a}=await O(async()=>{const{renderDashboard:n}=await import("./Dashboard-0HMpbBut.js");return{renderDashboard:n}},[]);await a(t,p)}async function V(t){Y=await h("meetings"),W(t)}function W(t){const a=p?.role==="admin";let n=[...Y].sort((e,r)=>e.date.localeCompare(r.date));x!=="all"&&(n=n.filter(e=>e.type===x)),t.innerHTML=`<div class="page active" id="page-meetings">
      ...
      ${n.map(e=>{const r=new Date(e.date+"T00:00:00");return`<div class="card">
          <div class="meeting-item">
            ...
            <div class="meet-body" style="flex:1">
              ...
              <div class="meet-tags"><span class="badge ${me(e.type)}">${ce(e.type)}</span></div>
            </div>
          </div>
        </div>`}).join("")||'<div class="empty"><span class="emic">📅</span><p>Sin reuniones en esta categoría</p></div>'}
    </div>
  </div>`,[["mf-all","all"],["mf-mid","midweek"],["mf-wk","weekend"],["mf-sp","special"]].forEach(([e,r])=>{document.getElementById(e)?.addEventListener("click",()=>{x=r,W(t)})}),t.querySelectorAll("[data-del-meet]").forEach(e=>{e.addEventListener("click",async()=>{confirm("¿Eliminar esta reunión?")&&(await z("meetings",e.dataset.delMeet),c("Eliminado","Reunión eliminada"),await V(t))})})}async function J(t){const a=await h("announcements"),n=p?.role==="admin";t.innerHTML=`<div class="page active" id="page-announcements">
    ...
    ${a.map(e=>`
      <div class="ann ${e.priority!=="normal"?e.priority:""}">
        ...
        <p>${e.body}</p>
        <div class="ann-meta">${pe(e.created_at)} <span class="badge ${e.priority==="urgent"?"b-rose":e.priority==="info"?"b-green":"b-sky"}">${e.priority}</span></div>
      </div>`).join("")||'<div class="empty"><span class="emic">📭</span><p>Sin anuncios</p></div>'}
  </div>`,t.querySelectorAll("[data-del-ann]").forEach(e=>{e.addEventListener("click",async()=>{confirm("¿Eliminar?")&&(await z("announcements",e.dataset.delAnn),c("Eliminado","Anuncio eliminado"),J(t))})})}async function K(t){const a=B(),e=(await h("assignments")).filter(s=>s.week===a),r=p?.role==="admin",i=p?.email||"",o=["#4a90d9","#2e9e6b","#c07820","#c0405a","#7a55c8"];t.innerHTML=`<div class="page active" id="page-assignments">
    <div class="section-hd">
      <h2 class="section-title">Asignaciones</h2>
      <span class="badge b-sky">Semana ${a}</span>
    </div>
    <div class="card">
      ${e.map((s,d)=>{const l=s.email===i,f=s.name.split(" ").map(w=>w[0]).join("").toUpperCase().slice(0,2),m=o[d%o.length];return`<div class="as-row" style="${l?"background:var(--sky-bg);border-radius:10px;padding:.75rem .6rem;margin:0 -.6rem":""}">
          <div class="as-left">
            <div class="avatar" style="background:${m}1a;color:${m};border:2px solid ${m}44">${f}</div>
            <div>
              <div style="font-weight:700;color:${l?"var(--sky3)":"var(--text)"}">${s.name} ${l?'<span style="font-size:.71rem;color:var(--sky);font-weight:600">(Tú)</span>':""}</div>
              <div style="font-size:.8rem;color:var(--text2)">${s.role}</div>
            </div>
          </div>
          <div style="display:flex;align-items:center;gap:.45rem">
            ${l?'<span class="badge b-sky">Tu asignación</span>':""}
            ${r?`<button class="btn-sm danger" data-del-as="${s.id}">✕</button>`:""}
          </div>
        </div>`}).join("")||'<div class="empty"><span class="emic">📋</span><p>Sin asignaciones esta semana</p></div>'}
    </div>
  </div>`,t.querySelectorAll("[data-del-as]").forEach(s=>{s.addEventListener("click",async()=>{await z("assignments",s.dataset.delAs),c("Eliminado","Asignación eliminada"),K(t)})})}function fe(t){t.innerHTML=`<div class="page active" id="page-programs">
  ...
  </div>`,document.getElementById("go-cleaning").addEventListener("click",()=>{A("cleaning"),I("cleaning")}),document.getElementById("go-work").addEventListener("click",()=>{A("workprogram"),I("workprogram")})}async function Z(t){const a=await h("cleaning"),n=p?.role==="admin";t.innerHTML=`<div class="page active" id="page-cleaning">
    <div class="section-hd">
  ...
  </div>`,document.getElementById("back-programs").addEventListener("click",()=>{A("programs"),I("programs")}),t.querySelectorAll("[data-del-cl]").forEach(e=>{e.addEventListener("click",async()=>{confirm("¿Eliminar?")&&(await z("cleaning",e.dataset.delCl),c("Eliminado","Turno eliminado"),Z(t))})})}async function Q(t){const a=await h("workprogram"),n=p?.role==="admin";t.innerHTML=`<div class="page active" id="page-workprogram">
    <div class="section-hd">
      <div style="display:flex;align-items:center;gap:.7rem">
  ...
  </div>`,document.getElementById("back-programs2").addEventListener("click",()=>{A("programs"),I("programs")}),t.querySelectorAll("[data-del-wk]").forEach(e=>{e.addEventListener("click",async()=>{confirm("¿Eliminar?")&&(await z("workprogram",e.dataset.delWk),c("Eliminado","Trabajo eliminado"),Q(t))})})}function ye(t){t.innerHTML=`<div class="page active" id="page-map">
  ...
  </div>`,setTimeout(()=>{if(b){b.invalidateSize();return}b=L.map("map").setView([9.93,-84.084],14),L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"© OpenStreetMap",maxZoom:19}).addTo(b);const a={available:"#2e9e6b","in-progress":"#c07820",completed:"#4a90d9"};ve.forEach(n=>{const e=L.divIcon({className:"",iconSize:[26,26],iconAnchor:[13,13],html:`<div style="width:26px;height:26px;border-radius:50%;background:${a[n.status]};border:3px solid white;box-shadow:0 2px 8px rgba(0,0,0,.3)"></div>`}),r=L.marker([n.lat,n.lng],{icon:e}).addTo(b).bindPopup(`<b>${n.name}</b><br>${n.group}<br><em>${n.notes}</em>`);G.push({marker:r,t:n})})},80),t.querySelectorAll("[data-filter]").forEach(a=>{a.addEventListener("click",()=>{t.querySelectorAll(".map-btn").forEach(e=>e.classList.remove("active")),a.classList.add("active");const n=a.dataset.filter;G.forEach(({marker:e,t:r})=>{n==="all"||r.status===n?!b.hasLayer(e)&&b.addLayer(e):b.hasLayer(e)&&b.removeLayer(e)})})})}async function X(t){if(p?.role==="admin")return he(t);const n=await h("reports"),e=n.filter(o=>o.email===p?.email&&o.year===y.getFullYear());t.innerHTML=`<div class="page active" id="page-reports">
    ...
    <div class="card">
      <div class="card-hd">
        ...
        <select id="report-month-sel" class="btn-sm">
          ${D.map((o,s)=>`<option value="${s}" ${s===y.getMonth()?"selected":""}>${o} ${y.getFullYear()}</option>`).join("")}
        </select>
      </div>
    ...
    <div class="card">
      ...
      <div class="month-grid">
        ${D.map((o,s)=>{const d=e.find(l=>l.month===s);return`<div class="month-cell ${d?"has-report":""}" title="${d?`Horas: ${d.hours}`:"Sin informe"}">
            <div class="mc-name">${o.slice(0,3)}</div>
            <div class="mc-status">${d?"✅":"○"}</div>
          </div>`}).join("")}
      </div>
    </div>
  </div>`;const r=document.getElementById("report-month-sel"),i=()=>{const o=parseInt(r.value),s=n.find(d=>d.email===p?.email&&d.year===y.getFullYear()&&d.month===o);document.getElementById("rp-hours").value=s?.hours||"",document.getElementById("rp-rv").value=s?.revisits||"",document.getElementById("rp-studies").value=s?.studies||"",document.getElementById("rp-videos").value=s?.videos||"",document.getElementById("rp-notes").value=s?.notes||""};i(),r.addEventListener("change",i),document.getElementById("btn-save-report").addEventListener("click",async()=>{const o=parseInt(r.value),s=parseInt(document.getElementById("rp-hours").value)||0,d=parseInt(document.getElementById("rp-rv").value)||0,l=parseInt(document.getElementById("rp-studies").value)||0,f=parseInt(document.getElementById("rp-videos").value)||0,m=document.getElementById("rp-notes").value.trim(),w=y.getFullYear();await H({email:p.email,year:w,month:o,hours:s,revisits:d,studies:l,videos:f,notes:m},{email:p.email,year:w,month:o}),c("Informe guardado",`${D[o]} ${w} · ${s} horas`),X(t)})}async function he(t){const a=await h("reports"),n=await M(),e=await j(),r=y.getFullYear(),i=y.getMonth(),o=a.filter(d=>d.year===r),s=a.filter(d=>d.year===r&&d.month===i);t.innerHTML=`<div class="page active" id="page-reports">
    <div class="section-hd"><h2 class="section-title">Informes de Predicación</h2></div>
    <div class="card">
      <div class="card-hd">
      ...
      <div style="overflow-x:auto">
        <table class="tbl">
          ...
          <tbody>
            ${n.map(d=>{const l=s.find(m=>m.email===d.email),f=e.find(m=>m.id===d.group_id);return`<tr>
                ...
                <td>${l?l.studies:"—"}</td>
                <td>${l?l.videos:"—"}</td>
                <td>${l?'<span class="badge b-green">✓ Enviado</span>':'<span class="badge b-rose">Pendiente</span>'}</td>
              </tr>`}).join("")}
          </tbody>
        </table>
      </div>
    ...
    <div class="card">
      ...
      <div style="display:flex;align-items:center;gap:2rem;flex-wrap:wrap">
        <div class="year-ring"><div class="num">${o.length}</div><div class="lbl">informes</div></div>
        <div class="g3" style="flex:1">
          <div class="stat"><div class="stat-icon">⏱️</div><div><div class="stat-val">${o.reduce((d,l)=>d+(l.hours||0),0)}</div><div class="stat-lbl">Horas totales</div></div></div>
          <div class="stat"><div class="stat-icon">📖</div><div><div class="stat-val">${o.reduce((d,l)=>d+(l.studies||0),0)}</div><div class="stat-lbl">Estudios</div></div></div>
          <div class="stat"><div class="stat-icon">🔄</div><div><div class="stat-val">${o.reduce((d,l)=>d+(l.revisits||0),0)}</div><div class="stat-lbl">Revisitas</div></div></div>
        </div>
      </div>
    </div>
  </div>`}async function v(t){const a=await M(),n=await j();t.innerHTML=`<div class="page active" id="page-admin">
        ...
        <table class="tbl">
          ...
          <tbody>
            ${a.map(e=>{const r=n.find(i=>i.id===e.group_id);return`<tr>
                ...
                <td>${e.email!==p?.email?`<button class="btn-sm" data-toggle-role="${e.email}" data-cur-role="${e.role}">${e.role==="admin"?"↓ Estándar":"↑ Admin"}</button>`:"—"}</td>
              </tr>`}).join("")}
          </tbody>
        </table>
      </div>
    </div>
  </div>`,document.getElementById("btn-add-group").addEventListener("click",async()=>{const e=document.getElementById("grp-name").value.trim(),r=document.getElementById("grp-captain").value.trim();if(!e){c("Error","Ingresa un nombre",!0);return}$().groups.push({id:"g"+Date.now(),name:e,captain:r}),require("./services/db.js").saveDS(),c("Grupo creado",e),document.getElementById("grp-name").value="",document.getElementById("grp-captain").value="",v(t)}),t.querySelectorAll("[data-del-grp]").forEach(e=>{e.addEventListener("click",async()=>{if(confirm("¿Eliminar este grupo?")){{$().groups=$().groups.filter(i=>i.id!==e.dataset.delGrp);const{saveDS:r}=await O(async()=>{const{saveDS:i}=await Promise.resolve().then(()=>ie);return{saveDS:i}},void 0);r()}c("Eliminado","Grupo eliminado"),v(t)}})}),document.getElementById("btn-add-meet").addEventListener("click",async()=>{const e=document.getElementById("m-title").value.trim(),r=document.getElementById("m-date").value,i=document.getElementById("m-time").value,o=document.getElementById("m-type").value,s=document.getElementById("m-desc").value.trim();if(!e||!r){c("Error","Completa título y fecha",!0);return}await E("meetings",{title:e,date:r,time:i,type:o,description:s}),c("Reunión guardada",`${e} · ${r}`),["m-title","m-desc"].forEach(d=>document.getElementById(d).value=""),v(t)}),document.getElementById("btn-add-ann").addEventListener("click",async()=>{const e=document.getElementById("an-title").value.trim(),r=document.getElementById("an-body").value.trim(),i=document.getElementById("an-pri").value;if(!e||!r){c("Error","Completa todos los campos",!0);return}await E("announcements",{title:e,body:r,priority:i}),c("Anuncio publicado",e),["an-title","an-body"].forEach(o=>document.getElementById(o).value=""),v(t)}),document.getElementById("btn-add-as").addEventListener("click",async()=>{const e=document.getElementById("as-name").value.trim(),r=document.getElementById("as-email").value.trim().toLowerCase(),i=document.getElementById("as-role").value.trim(),o=document.getElementById("as-week").value;if(!e||!i||!o){c("Error","Completa los campos requeridos",!0);return}await E("assignments",{name:e,email:r,role:i,week:o}),c("Asignación guardada",`${e} – ${i}`),["as-name","as-email","as-role"].forEach(s=>document.getElementById(s).value=""),v(t)}),document.getElementById("btn-add-cl").addEventListener("click",async()=>{const e=document.getElementById("cl-who").value.trim(),r=document.getElementById("cl-date").value,i=document.getElementById("cl-notes").value.trim();if(!e||!r){c("Error","Completa responsable y fecha",!0);return}await E("cleaning",{who:e,date:r,notes:i}),c("Turno publicado",e),["cl-who","cl-notes"].forEach(o=>document.getElementById(o).value=""),document.getElementById("cl-date").value="",v(t)}),document.getElementById("btn-add-wk").addEventListener("click",async()=>{const e=document.getElementById("wk-title").value.trim(),r=document.getElementById("wk-date").value,i=document.getElementById("wk-who").value.trim(),o=document.getElementById("wk-notes").value.trim();if(!e||!r){c("Error","Completa el trabajo y la fecha",!0);return}await E("workprogram",{title:e,date:r,who:i,notes:o}),c("Trabajo publicado",e),["wk-title","wk-who","wk-notes"].forEach(s=>document.getElementById(s).value=""),document.getElementById("wk-date").value="",v(t)}),document.getElementById("btn-refresh-users").addEventListener("click",()=>v(t)),t.querySelectorAll("[data-set-grp]").forEach(e=>{e.addEventListener("change",async()=>{await q(e.dataset.setGrp,e.value),c("Grupo asignado","")})}),t.querySelectorAll("[data-toggle-role]").forEach(e=>{e.addEventListener("click",async()=>{const r=e.dataset.curRole==="admin"?"user":"admin";await U(e.dataset.toggleRole,r),c("Rol actualizado",`→ ${r}`),v(t)})})}let S;window.addEventListener("beforeinstallprompt",t=>{t.preventDefault(),S=t,document.getElementById("install-bar")?.classList.add("show")});document.getElementById("btn-install")?.addEventListener("click",async()=>{S&&(S.prompt(),await S.userChoice,S=null),document.getElementById("install-bar")?.classList.remove("show")});"serviceWorker"in navigator&&navigator.serviceWorker.register("/Proyect2/sw.js").catch(()=>{});window.__showAuth();export{y as N,$ as a,B as c,pe as f,h as g,ce as t};
